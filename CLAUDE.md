...
- **Rule 08 — Icons come from a real icon library; never hand-drawn, and they must contrast with their surface.** In Playground, use `Icons.*` from [system/icons.jsx](system/icons.jsx) (the kit is sourced from Lucide: 1.5px stroke, 24px viewBox, **currentColor** — the icon inherits the parent's `color`). In Paper, drop in icons from a real library — Lucide first (matches the kit), Phosphor or Heroicons only if Lucide can't express the metaphor. Never compose icons from rectangles, lines, or freehand shapes. If the icon you need isn't available, raise the gap rather than inlining a one-off SVG. **Contrast is non-negotiable:** because icons render in `currentColor`, the icon's container `color` must be set to a foreground that contrasts with the surface it sits on. A black icon on a black button (or a bone icon on a bone surface) is the failure mode — set `color` to the opposite-side foreground (`--bone` family on dark surfaces, `--dark` family on light surfaces). Audit visually after placing every icon; black-on-black or bone-on-bone icons are P0.
- **Rule 09 — Design-system fonts only; in Paper, preflight the font set.** Wireframes use TWK Lausanne Pan (300 / 550 / 700) — nothing else. Apercu is display-only on the home tile; Inter is not in the system; Space Mono is removed and must never be reintroduced (the legacy `--font-mono` token is now an alias of `--font-ui` so old callers keep resolving to Lausanne). In Playground, use `var(--font-ui)` / `var(--font-note)` — never name a face directly, and do not introduce a new monospace token. In Paper, before any typographic styling, call `get_font_family_info`; if Lausanne Pan is missing from the file, **stop** and surface that to the PM (and the human, via the PM) before composing. Do not substitute system-ui, Inter, Space Mono, or any other fallback.
